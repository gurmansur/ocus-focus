...
export class InitialMigration1741823072233 implements MigrationInterface {
  name = 'InitialMigration1741823072233';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`COMENTARIOS\` (\`cmn_id\` int NOT NULL AUTO_INCREMENT, \`cmn_comentario\` varchar(255) NOT NULL, \`cmn_criado_em\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`cmn_modificado_em\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, \`cmn_deletado\` tinyint NOT NULL DEFAULT 0, \`fk_usuario_id\` int NULL, \`fk_user_story\` int NULL, PRIMARY KEY (\`cmn_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`SWIMLANES\` (\`SWI_ID\` int NOT NULL AUTO_INCREMENT, \`SWI_NOME\` varchar(30) NOT NULL, \`SWI_VERTICAL\` tinyint(1) NOT NULL DEFAULT '0', \`SWI_COR\` varchar(7) NOT NULL DEFAULT '#6d28d9', \`SWI_CRIADO_EM\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`SWI_ATUALIZADO_EM\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`FK_KAN_ID\` int NULL, PRIMARY KEY (\`SWI_ID\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`TAGS\` (\`tag_id\` int NOT NULL AUTO_INCREMENT, \`tag_nome\` varchar(20) NOT NULL, \`tag_cor\` varchar(7) NOT NULL, PRIMARY KEY (\`tag_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`USER_STORY_TAGS\` (\`tAGSTagId\` int NOT NULL, \`uSERSTORIESUSTID\` int NOT NULL, INDEX \`IDX_2c0af47dcb6f80deeaa0d6960e\` (\`tAGSTagId\`), INDEX \`IDX_927363b612333792e05e4e90ae\` (\`uSERSTORIESUSTID\`), PRIMARY KEY (\`tAGSTagId\`, \`uSERSTORIESUSTID\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`COMENTARIOS\` ADD CONSTRAINT \`FK_344f8118376d5a82e4ecb4d4923\` FOREIGN KEY (\`fk_usuario_id\`) REFERENCES \`COLABORADORES\`(\`COL_ID\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`USER_STORY_TAGS\` ADD CONSTRAINT \`FK_2c0af47dcb6f80deeaa0d6960e2\` FOREIGN KEY (\`tAGSTagId\`) REFERENCES \`TAGS\`(\`tag_id\`) ON DELETE CASCADE ON UPDATE CASCADE`,
    );
    await queryRunner.query(
      `ALTER TABLE \`USER_STORY_TAGS\` ADD CONSTRAINT \`FK_927363b612333792e05e4e90aea\` FOREIGN KEY (\`uSERSTORIESUSTID\`) REFERENCES \`USER_STORIES\`(\`UST_ID\`) ON DELETE CASCADE ON UPDATE CASCADE`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `INSERT INTO \`TAGS\` (\`TAG_ID\`,\`TAG_NOME\`, \`TAG_COR\`) VALUES (1,'Documentação', '#34e161')`,
    );
    await queryRunner.query(
      `INSERT INTO \`TAGS\` (\`TAG_ID\`,\`TAG_NOME\`, \`TAG_COR\`) VALUES (2,'Desenvolvimento', '#346de1')`,
    );
    await queryRunner.query(
      `INSERT INTO \`TAGS\` (\`TAG_ID\`,\`TAG_NOME\`, \`TAG_COR\`) VALUES (3,'Criação de testes', '#6f34e1')`,
    );
    await queryRunner.query(
      `INSERT INTO \`TAGS\` (\`TAG_ID\`,\`TAG_NOME\`, \`TAG_COR\`) VALUES (4,'Teste', '#e1345e')`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`USER_STORY_TAGS\` DROP FOREIGN KEY \`FK_927363b612333792e05e4e90aea\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`USER_STORY_TAGS\` DROP FOREIGN KEY \`FK_2c0af47dcb6f80deeaa0d6960e2\``,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DROP INDEX \`IDX_927363b612333792e05e4e90ae\` ON \`USER_STORY_TAGS\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_2c0af47dcb6f80deeaa0d6960e\` ON \`USER_STORY_TAGS\``,
    );
    await queryRunner.query(`DROP TABLE \`USER_STORY_TAGS\``);
